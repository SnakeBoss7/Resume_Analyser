...
export default function Header({ pages, links, icons }) {
  ...
  return (
    <header
      ...
      }  border-black-600 flex flex-col ${
        isMenuOpen ? "h-[500px] fixed" : ""
      } `}
    >
      <div className=" mx-auto container w-full flex justify-between ">
        ...
        <div className="right flex items-center gap-5">
          ...
          <Link className="text-bg_c font-bold border  border-bg_c  px-4 py-2 rounded-md  hover:bg-gradient-to-r hover:from-blue-500 hover:via-blue-500 hover:to-purple-600 hover:text-white hover:border-white" to='/signIn'>Get Started</Link>
          <Checkbox toggle={toggleNav} isOpen={isMenuOpen} />
        </div>
      </div>
      {isMenuOpen && (
        <div className="div  mt-20 flex flex-col divide-y-4 divide-y-reverse divide-gray-200  gap-5 h-[100%] ">
          {pages.map((page, idx) => {
            const Icon = icons && icons[idx];
            return (
              <Link
                key={idx}
                to={`/${links[idx]}`}
                className="h-fit flex w-30   tracking-tight font-semibold  text-2xl justify-center text-gray-600 items-center gap-2 font-bold"
              >
                {Icon && <Icon className="text-4xl text-gray-600" />}
                {page}
              </Link>
            );
          })}
